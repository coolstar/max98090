	...
	if (length > DEFAULT_SPB_BUFFER_SIZE)
	{
		...
		if (!NT_SUCCESS(status))
		{
			MaxmPrint(
				DEBUG_LEVEL_ERROR,
				DBG_IOCTL,
				"Error allocating memory for Spb write - %x\n",
				status);
			goto exit;
		}
...
--*/
{
	...
	if (!NT_SUCCESS(status))
	{
		MaxmPrint(
			DEBUG_LEVEL_ERROR,
			DBG_IOCTL,
			"Error setting address pointer for Spb read - %x\n",
			status);
		goto exit;
	}
	...
	if (Length > DEFAULT_SPB_BUFFER_SIZE)
	{
		...
		if (!NT_SUCCESS(status))
		{
			MaxmPrint(
				DEBUG_LEVEL_ERROR,
				DBG_IOCTL,
				"Error allocating memory for Spb read - %x\n",
				status);
			goto exit;
		}
